...
chrome.storage.local.get("backgroundActiveScrape", (data) => {
  ...
  if (data && data.backgroundActiveScrape) {
    ...
    console.log("Restored active background scrape state from storage:", activeScrape);
  }
...
async function checkAmazonAuth() {
  try {
    ...
    const response = await fetch("https://www.amazon.com/cpe/yourpayments/transactions", {
      method: "GET",
      credentials: "include"
    });
    
    const finalUrl = response.url.toLowerCase();
    if (finalUrl.includes("/signin") || finalUrl.includes("/login") || finalUrl.includes("/register") || finalUrl.includes("ap/signin")) {
...
chrome.tabs.onRemoved.addListener((tabId) => {
  if (activeScrape.tabId === tabId) {
    console.log("DataPrime: Scraper background tab was manually closed by the user.");
    if (activeScrape.status === "RUNNING" || activeScrape.status === "ITEMIZING") {
...
function handleBackgroundMessage(request, sender, sendResponse) {
    ...
    chrome.storage.local.remove("activeScrapeSession", () => {
      ...
      checkAmazonAuth().then((authenticated) => {
        if (!authenticated) {
          ...
          activeScrape.status = "ERROR";
          activeScrape.message = "You are not logged in to Amazon. Please log in in the opened tab and try again.";
          broadcastToAll("SCRAPE_STATE_CHANGED", activeScrape);
  ...
  if (request.action === "CONTENT_SCRIPT_READY") {
    if (sender.tab && sender.tab.id === activeScrape.tabId && activeScrape.status === "RUNNING") {
      console.log(`DataPrime background.js: Content script is alive in scraper tab ${sender.tab.id}. Triggering scraper start message...`);
      ...
      chrome.tabs.sendMessage(activeScrape.tabId, {
        action: "START_SCRAPE",
        ...
        if (chrome.runtime.lastError) {
          console.error("Failed to start scraper in background tab:", chrome.runtime.lastError.message);
          activeScrape.status = "ERROR";
          activeScrape.message = "Failed to communicate with Amazon: " + chrome.runtime.lastError.message;
          broadcastToAll("SCRAPE_STATE_CHANGED", activeScrape);
  ...
  if (request.action === "STOP_SCRAPE") {
    ...
    if (activeScrape.tabId) {
      chrome.tabs.sendMessage(activeScrape.tabId, { action: "STOP_SCRAPE" }, () => {
...
async function seedDemoData() {
  const categories = ["Electronics", "Kitchen", "Apparel", "Office Supplies", "Groceries", "Books", "Streaming", "Home Goods"];
  const sellers = ["Amazon.com", "Anker Direct", "Spreetail", "Whole Foods Market", "Patagonia", "Logitech Inc.", "Digital Services"];
  ...
  for (let i = 0; i < 25; i++) {
    ...
    for (let j = 0; j < itemCount; j++) {
      const itemPrice = j === itemCount - 1 ? remainingAmount : parseFloat((remainingAmount / (itemCount - j) * (0.6 + Math.random() * 0.4)).toFixed(2));
      ...
      if (category === "Electronics") {
        itemTitle = ["Anker USB-C Power Hub 100W", "Logitech MX Master 3S Wireless Mouse", "Sony WH-1000XM4 Noise Cancelling Headphones", "Kindle Paperwhite (16 GB)"][Math.floor(Math.random() * 4)];
      } else if (category === "Kitchen") {
        itemTitle = ["Instant Pot Duo 7-in-1 Smart Cooker", "Hydro Flask Wide Mouth Water Bottle", "Cosori Air Fryer Max XL 5.8 Qt", "Bodum Chambord French Press"][Math.floor(Math.random() * 4)];
      } else if (category === "Groceries") {
        itemTitle = ["Organic Fuji Apples (3lb Bag)", "LaCroix Sparkling Water 24-Pack", "Organic Creamy Peanut Butter 28oz", "365 Everyday Value Olive Oil"][Math.floor(Math.random() * 4)];
      } else if (category === "Apparel") {
        itemTitle = ["Patagonia Better Sweater Fleece Jacket", "Levis 511 Slim Fit Men's Jeans", "Champion Powerblend Fleece Hoodie", "Darn Tough Merino Wool Hiking Socks"][Math.floor(Math.random() * 4)];
      }
    ...
    mockTransactions.push({
      ...
      paymentMethod: ["Visa (*4321)", "MasterCard (*9876)", "Amex (*1002)", "Amazon Gift Card"][Math.floor(Math.random() * 4)],
      items
    });
  }
