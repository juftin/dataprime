...
test("DataPrime Parser Suite", async (t) => {
  await t.test(
    "1. parseOrderDetailsHtmlRegexFallback() - Digital Subscription Parsing",
    () => {
  ...
  await t.test(
    "2. parseOrderDetailsHtmlRegexFallback() - Grocery Order Details Parsing",
    () => {
  ...
  await t.test(
    "3. parseTransactionElement() - Standard Transactions parsing",
    () => {
      resetScrapingState();
  ...
  await t.test(
    "4. parseTransactionElement() - Refund Transactions parsing",
    () => {
      resetScrapingState();
  ...
  await t.test(
    "5. parseTransactionElement() - Split charges stateful sequence ID tracking",
    () => {
      resetScrapingState();
  ...
  await t.test(
    "6. parsePaymentMethod() - Brand extraction and trailing card digits",
    () => {
      assert.strictEqual(
        parsePaymentMethod("Visa ending in 4821"),
        "Visa (*4821)",
      );
      assert.strictEqual(parsePaymentMethod("MC *5566"), "MasterCard (*5566)");
      assert.strictEqual(
        parsePaymentMethod("Amex ending in 1002"),
        "Amex (*1002)",
      );
      assert.strictEqual(
  ...
  await t.test(
    "7. isElementDisabled() - Disabled navigation controls detection",
    () => {
  ...
  await t.test(
    "8. isElementVisible() - Offset dimension and client rect check",
    () => {
      const visibleEl1 = {
        offsetWidth: 10,
        offsetHeight: 0,
  ...
  await t.test(
    "9. parsePaymentMethod() - Advanced brand matching fallback cases",
    () => {
  ...
  await t.test(
    "10. parseOrderDetailsHtmlRegexFallback() - Quantity parsing edge cases",
    () => {
  ...
  await t.test(
    "11. parseOrderDetailsHtmlRegexFallback() - Seller parsing and image extraction variations",
    () => {
      const mockHtml = `
  ...
  await t.test(
    "12. parseTransactionElement() - Grocery order URL redirection overrides",
    () => {
      resetScrapingState();
  ...
  await t.test(
    "13. startScrapingLoop loop protection - Duplicate page content hash matching",
    () => {
      ...
      const isPageIdenticalMatches =
        ...
        currentPageBaseKeys.length ===
          lastPageTransactionBaseKeysMatches.length &&
      ...
      const isPageIdenticalDiffers =
        ...
        currentPageBaseKeys.length ===
          lastPageTransactionBaseKeysDiffers.length &&
        currentPageBaseKeys.every(
  ...
  await t.test(
    "14. parseTransactionElement() - Missing or invalid fields handling",
    () => {
      resetScrapingState();
      ...
      const mockElBadDate = {
        ...
        querySelector(selector) {
          if (selector.includes("date"))
            return { innerText: "Not A Date String" };
          return null;
        },
        querySelectorAll() {
          return [];
  ...
  await t.test(
    "15. parseOrderDetailsHtmlRegexFallback() - Seller fallback when absent",
    () => {
  ...
  await t.test(
    "16. parseOrderDetailsHtmlRegexFallback() - Multiple digital synthetic ASIN indexing bounds",
    () => {
  ...
  await t.test(
    "17. startScrapingLoop() - Date range accumulation boundary logic",
    () => {
      resetScrapingState();
  ...
  await t.test(
    "19. parseTransactionElement() - Current-year transaction dates parsing (no year present)",
    () => {
      resetScrapingState();
  ...
  await t.test(
    "20. findNextButton() - Next page control checking & circular link filtering",
    () => {
   ...
   */
  await t.test(
    "21. parseOrderDetailsHtmlRegexFallback() - Skip promotional/advertisement items",
    () => {
      const mockHtml = `
      <div class="orderDetails">
        <!-- Valid purchase item -->
