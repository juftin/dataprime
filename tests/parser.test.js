...
test('DataPrime Parser Suite', async (t) => {

  await t.test('1. parseOrderDetailsHtmlRegexFallback() - Digital Subscription Parsing', () => {
  ...
  await t.test('2. parseOrderDetailsHtmlRegexFallback() - Grocery Order Details Parsing', () => {
  ...
  await t.test('3. parseTransactionElement() - Standard Transactions parsing', () => {
    resetScrapingState();
    ...
    assert.strictEqual(parsed.amount, 45.50); // Charges should be stored as positive amounts
    assert.strictEqual(parsed.paymentMethod, "Visa (*9876)");
    assert.strictEqual(parsed.id, "114-9876543-2109876-2026-01-25-45.50-0");
  });

  await t.test('4. parseTransactionElement() - Refund Transactions parsing', () => {
    resetScrapingState();
    ...
    assert.ok(parsed);
    assert.strictEqual(parsed.amount, -15.20); // Refunds should be stored as negative amounts
    assert.strictEqual(parsed.id, "114-9876543-2109876-2026-02-12-15.20-0");
  });

  await t.test('5. parseTransactionElement() - Split charges stateful sequence ID tracking', () => {
    resetScrapingState();
  ...
  await t.test('6. parsePaymentMethod() - Brand extraction and trailing card digits', () => {
    ...
    assert.strictEqual(parsePaymentMethod("Discover card ending in 9081"), "Discover (*9081)");
    assert.strictEqual(parsePaymentMethod("Paid with Amazon Gift Card"), "Amazon Gift Card");
    assert.strictEqual(parsePaymentMethod("Amazon Account Balance and Visa 1122"), "Visa (*1122)");
  });

  await t.test('7. isElementDisabled() - Disabled navigation controls detection', () => {
    const mockDisabledEl = {
      disabled: true,
      getAttribute(attr) { return attr === "disabled" ? "true" : null; },
    ...
    const mockClassDisabledEl = {
      ...
      classList: {
        contains(cls) { return cls === "a-disabled" || cls === "a-button-disabled"; }
      },
      closest() { return null; }
    };
    ...
    const mockAriaDisabledEl = {
      disabled: false,
      getAttribute(attr) { return attr === "aria-disabled" ? "true" : null; },
      classList: { contains() { return false; } },
      closest() { return null; }
    };
    ...
    const mockChildAriaDisabledEl = {
      ...
      getAttribute() { return null; },
      classList: { contains() { return false; } },
      closest(sel) { return sel.includes('aria-disabled') ? {} : null; }
    };
  ...
  await t.test('8. isElementVisible() - Offset dimension and client rect check', () => {
    const visibleEl1 = { offsetWidth: 10, offsetHeight: 0, getClientRects() { return []; } };
    const visibleEl2 = { offsetWidth: 0, offsetHeight: 15, getClientRects() { return []; } };
    const visibleEl3 = { offsetWidth: 0, offsetHeight: 0, getClientRects() { return [{}]; } };
    const hiddenEl = { offsetWidth: 0, offsetHeight: 0, getClientRects() { return []; } };
  ...
  await t.test('9. parsePaymentMethod() - Advanced brand matching fallback cases', () => {
    // Unbranded card digits matching
    assert.strictEqual(parsePaymentMethod("Paid with Card ending in 7744"), "Card (*7744)");
    // MC lower/upper case matching
    assert.strictEqual(parsePaymentMethod("charged to mastercard 3322"), "MasterCard (*3322)");
    assert.strictEqual(parsePaymentMethod("charged to mc 5544"), "MasterCard (*5544)");
    // Amazon Account Balance matching
    assert.strictEqual(parsePaymentMethod("Paid with checking account"), "Amazon Account Balance");
  });

  await t.test('10. parseOrderDetailsHtmlRegexFallback() - Quantity parsing edge cases', () => {
  ...
  await t.test('11. parseOrderDetailsHtmlRegexFallback() - Seller parsing and image extraction variations', () => {
    const mockHtml = `
    ...
    assert.strictEqual(items[0].imageUrl, "https://images-na.ssl-images-amazon.com/images/I/71xyz.jpg");
  });

  await t.test('12. parseTransactionElement() - Grocery order URL redirection overrides', () => {
    resetScrapingState();
    ...
    const mockEl = {
      ...
      querySelector(selector) {
        ...
        if (selector.includes("orderID") || selector.includes("order-details") || selector.includes("a[")) {
          return { href: "https://www.amazon.com/gp/your-account/order-details?orderID=112-8888888-8888888" };
        }
        return null;
      },
      querySelectorAll() { return []; }
  ...
  await t.test('13. startScrapingLoop loop protection - Duplicate page content hash matching', () => {
    const pageTransactions = [
      { id: "tx-1-0", baseKey: "tx-1", date: "2026-06-01", amount: 10.00 },
      { id: "tx-2-0", baseKey: "tx-2", date: "2026-06-01", amount: 20.00 }
    ];
    ...
    const isPageIdenticalMatches = currentPageBaseKeys.length > 0 && 
                                  lastPageTransactionBaseKeysMatches && 
                                  currentPageBaseKeys.length === lastPageTransactionBaseKeysMatches.length &&
                                  currentPageBaseKeys.every((key, idx) => key === lastPageTransactionBaseKeysMatches[idx]);
    ...
    const isPageIdenticalDiffers = currentPageBaseKeys.length > 0 && 
                                  lastPageTransactionBaseKeysDiffers && 
                                  currentPageBaseKeys.length === lastPageTransactionBaseKeysDiffers.length &&
                                  currentPageBaseKeys.every((key, idx) => key === lastPageTransactionBaseKeysDiffers[idx]);
                                  
    assert.strictEqual(isPageIdenticalDiffers, false);
  });

  await t.test('14. parseTransactionElement() - Missing or invalid fields handling', () => {
    resetScrapingState();
  ...
  await t.test('15. parseOrderDetailsHtmlRegexFallback() - Seller fallback when absent', () => {
  ...
  await t.test('16. parseOrderDetailsHtmlRegexFallback() - Multiple digital synthetic ASIN indexing bounds', () => {
    ...
    const items = parseOrderDetailsHtmlRegexFallback(mockHtmlMultipleDigital, "D01-99999");
    assert.strictEqual(items.length, 2);
  ...
  await t.test('17. startScrapingLoop() - Date range accumulation boundary logic', () => {
    resetScrapingState();
    ...
    const t1 = { id: "tx-1", date: "2026-05-22", amount: 10.00 }; // Case A: Too new (> endDate)
    const t2 = { id: "tx-2", date: "2026-05-15", amount: 15.00 }; // Case B: In-range (added!)
    const t3 = { id: "tx-3", date: "2026-05-05", amount: 20.00 }; // Case C: Too old (< startDate, triggers finish)
  ...
  await t.test('18. findPreviousButton() - Page 1 checking assertions', () => {
    // We temporarily override standard global document queries to simulate pagination states
    const originalQuerySelector = globalThis.document ? globalThis.document.querySelector : null;
    const originalQuerySelectorAll = globalThis.document ? globalThis.document.querySelectorAll : null;
  ...
  await t.test('19. parseTransactionElement() - Current-year transaction dates parsing (no year present)', () => {
    resetScrapingState();
    ...
    const expectedDate = new Date(Date.parse(`May 25, ${expectedYear}`)).toISOString().split('T')[0];
    assert.strictEqual(parsed.date, expectedDate);
    assert.strictEqual(parsed.amount, 19.99);
  });

  await t.test('20. findNextButton() - Next page control checking & circular link filtering', () => {
    ...
    globalThis.document = {
      ...
      createElement() {
        return {
          id: "", style: {}, appendChild() {}, addEventListener() {}, remove() {},
          innerHTML: "", innerText: "", querySelector() { return null; }, querySelectorAll() { return []; }
        };
      },
      querySelector(selector) {
   ...
   */
  await t.test('21. parseOrderDetailsHtmlRegexFallback() - Skip promotional/advertisement items', () => {
    const mockHtml = `
      <div class="orderDetails">
        <!-- Valid purchase item -->
        <div class="a-row">
    ...
    const items = parseOrderDetailsHtmlRegexFallback(mockHtml, "111-2222222-3333333");
    assert.strictEqual(items.length, 1);
